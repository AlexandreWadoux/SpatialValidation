...
for (sampling in 1:nIter2){
  ...
  pt.SRS <- s_df[sample(nrow(s_df), SampSize), ]
  valuetable <- na.omit(as.data.frame(pt.SRS))
  ...
  for (j in 1:NB_iteration)     # from 1 to the number of test pixels in all groups
  {
    ...
    while(Point_in_range == FALSE)
    {
      ...
      # Check if the test pixel fall within the range of all predictors in the training set
  ...
  for (i in 1:length(x))   # compute validation statistics by exclusion buffer radius r
  {
    ...
    flds <- createFolds(row.names(tmp), k = NB_groups, list = FALSE, returnTrain = FALSE)
    ...
    ME_RF_FULL_list = c()
    MEC_RF_FULL_list = c()
    # Compute the validation statistics by groups of test pixels
  ...
  save(res.ME, res.RMSE, res.r2, res.MEC, file = paste0('res_random_testnew', sampling, '.Rdata'))
}

save(res.ME, res.RMSE, res.r2, res.MEC, file = 'res_random_500.Rdata')
