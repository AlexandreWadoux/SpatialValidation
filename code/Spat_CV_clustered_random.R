...
for (sampling in 1:nIter2){ # for each sample
  
  ############
  ############  Generate the clustered sample
  ############
  
  # take only the coordinates
  sp.grid <- s_df[c('x', 'y')]
  
  # make a spatial clustering on the coordinates using k-means
  #set.seed(1)
  tkmean <- kmeans(sp.grid, centers = 100, nstart = 10)
  sp.grid$psu <- tkmean$cluster
  
  # take a clustered sampling
  mysample <- twostage(sframe = sp.grid, psu="psu", n=20, m=25)
  coordinates(mysample) <- ~x+y
  
  ## plot the sampling
  # sp.grid2 <- sp.grid
  # coordinates(sp.grid2) <- ~x+y
  # gridded(sp.grid2) <- T ; sp.grid2 <- raster(sp.grid2)
  # plot(sp.grid2)
  # plot(mysample, add=T)
  
  # make a table with the covariate values
  valuetable.twoStage <- raster::extract(s, mysample, sp=T, df=T, na.rm = T)
  valuetable <- na.omit(as.data.frame(valuetable.twoStage))
  ...
  # build a RF model using the clustered sampling 
  RF <- ranger(formula = form_RF, data = valuetable)
  ...
  {
    # Select data corresponding to exclusion radius r at iteration i
    ...
    R2_RF_FULL_list = c()
    # Generate empty vectors for mean and sd of other validation statistics by groups of test pixels FOR EXCLUSION RADIUS AT ITERATION i
    RMSE_null_list = c()
    RMSE_RF_FULL_list = c()
    ME_RF_FULL_list = c()
    MEC_RF_FULL_list = c()
    # Compute the  RMSPE by groups of test pixels
  ...
  # compute for all validation technique the difference from the population validation statistics
  ...
  res.ME[sampling,][1:4] <- outtab.val.ME.sub[,c(1:4)]
  res.RMSE[sampling,][1:4] <- outtab.val.RMSE.sub[,c(1:4)]
  res.r2[sampling,][1:4] <- outtab.val.r2.sub[,c(1:4)]
  res.MEC[sampling,][1:4] <- outtab.val.MEC.sub[,c(1:4)]
  
  print(paste0(sampling, ' DONE, OUT OF ', nIter2))
  
  # save a the (temp, for each sampling) final results in a Rdata file
  save(res.ME, res.RMSE, res.r2, res.MEC, file = paste0('res_clustered_random_testnew', sampling, '.Rdata'))
  
}

save(res.ME, res.RMSE, res.r2, res.MEC, file = 'res_clustered_random_500.Rdata')

# plot the results of the validation statistics for the clustered sampling
